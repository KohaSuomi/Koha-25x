...
<xsl:stylesheet version="1.0"
  ...
  <xsl:include href="MARC21Languages.xsl"/>
    ...
    <xsl:template name="host-item-entries">
        <xsl:param name="UseControlNumber"/>
        <xsl:variable name="controlField003" select="str:encode-uri(marc:controlfield[@tag=003], true())"/>
        <!-- 773 -->
            ...
            <xsl:for-each select="marc:datafield[@tag=773]">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$f773cni != ''">
                        <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=Control-number,ext:"<xsl:value-of select="$f773cn"/>" AND cni,ext:"<xsl:value-of select="$f773cni"/>"</xsl:attribute>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=Control-number,ext:"<xsl:value-of select="$f773cn"/>" AND cni,ext:"<xsl:value-of select="$controlField003"/>"</xsl:attribute>
                    </xsl:otherwise>
                </xsl:choose>
