            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        if (
                            ...
                            if (row.status == "T") {
                                select +=
                                    '" disabled="disabled"><option value="T" selected="selected">' + __("In transit") + "</option></select>";
                            } else if (row.status == "P") {
                                select +=
                                    '" disabled="disabled"><option value="P" selected="selected">' + __("In processing") + "</option></select>";
                            } else if (row.status == "W") {
                                select +=
                                    '" disabled="disabled"><option value="W" selected="selected">' + __("Waiting") + "</option></select>";
                            } else {
                                if (
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        let buttons =
                            '<a class="hold-arrow move-hold ' + table_class + '" title="' + __("Move hold up") + '" href="#" data-move-hold="up" data-priority="' +
                            row.priority +
                            '" reserve_id="' +
                            row.hold_id +
                            '"><i class="fa fa-lg icon-move-hold-up" aria-hidden="true"></i></a>';
                        buttons +=
                            '<a class="hold-arrow move-hold ' + table_class + '" title="' + __("Move hold to top") + '" href="#" data-move-hold="top" data-priority="' +
                            row.priority +
                            '" reserve_id="' +
                            row.hold_id +
                            '"><i class="fa fa-lg icon-move-hold-top" aria-hidden="true"></i></a>';
                        buttons +=
                            '<a class="hold-arrow move-hold ' + table_class + '" title="' + __("Move hold to bottom") + '" href="#" data-move-hold="bottom" data-priority="' +
                            row.priority +
                            '" reserve_id="' +
                            row.hold_id +
                            '"><i class="fa fa-lg icon-move-hold-bottom" aria-hidden="true"></i></a>';
                        buttons +=
                            '<a class="hold-arrow move-hold ' + table_class + '" title="' + __("Move hold down") + '" href="#" data-move-hold="down" data-priority="' +
                            row.priority +
                            '" reserve_id="' +
                            row.hold_id +
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        if (row.status) {
                            ...
                            let barcode = row.item.external_id ? row.item.external_id : __("No barcode");
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function (data, type, row, meta) {
                        if (row.item_id || !CAN_user_reserveforothers_modify_holds_priority) {
                            return null;
                        } else {
    ...
    $(table_id).on("draw.dt", function () {
        ...
        $(".cancel_selected_holds").html(
            MSG_CANCEL_SELECTED.format(
                JSON.parse(localStorage.selectedHolds || "[]").length
            )
        );
        ...
        $(".holds_table .select_hold_all").on("click", function () {
            ...
            $("input.select_hold", allCheckboxes).each(function () {
                pageIds.push($(this).data("id"));
            });

            if (isChecked) {
                // Add all page IDs to the selection, avoiding duplicates
                selected = Array.from(new Set(selected.concat(pageIds)));
                $("input.select_hold", allCheckboxes).prop("checked", true);
                $("input.select_hold", allCheckboxes).each(function () {
                    $(this).parent().parent().addClass("selected");
                });
            } else {
                // Remove all page IDs from the selection
            ...
            $(".cancel_selected_holds").html(
                MSG_CANCEL_SELECTED.format(selected.length)
            );
        });
        ...
        $(".cancel_selected_holds").on("click", function (e) {
            e.preventDefault();
            if (localStorage.selectedHolds.length) {
