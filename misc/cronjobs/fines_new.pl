...
# Batch load all patrons at once to avoid N+1 queries
my %patrons_by_id = ();
if ( @{$overdues} ) {
    my @unique_borrowers = keys %{{ map { $_->{borrowernumber} => 1 } @{$overdues} }};
    my $patrons = Koha::Patrons->search(
        { borrowernumber => \@unique_borrowers },
        { columns => [qw( borrowernumber branchcode categorycode cardnumber surname firstname email phone address city )] }
    );
    while ( my $patron = $patrons->next ) {
        $patrons_by_id{ $patron->borrowernumber } = $patron;
    }
}
...
for my $overdue ( @{$overdues} ) {
    ...
    my $patron = $patrons_by_id{ $overdue->{borrowernumber} };
    next unless $patron;  # Skip if patron not found
